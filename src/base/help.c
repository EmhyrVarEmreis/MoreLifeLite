...
void help_print_options( options* o ) {
	printf( "\tCYCLES\t\t%-14d  STEP\t\t%d\n\tPNG\t\t%c\t\tSVG\t\t%c\n\tTXT\t\t%c\t\tGNUPLOT_FILE\t%c\n\tHEAT_MAP\t%c\t\tRULES\t\t%s\n",
			o->options[8],
			o->options[9],
			TOOL_OPT(o->options[0]),
			TOOL_OPT(o->options[1]),
			TOOL_OPT(o->options[7]),
			TOOL_OPT(o->options[10]),
			TOOL_OPT(o->options[2]),
			( o->strings[3][0] == '\0') ? "23/3" : (rules_is_valid( o->strings[3] )) ? o->strings[3] : "?/?"
		);
}
